...
metrics_sc_perm <- read_rds(here::here("ignore", "outputs", "studyforrest_retinotopy_sc_pls_ring_expand_metrics_permuted.rds"))
```
...
```{r q2-perm-pvals}
metrics_sc_perm %>% 
  select(fold_num, iteration, perf_overall) %>%
  unnest(perf_overall) %>% 
  filter(encoding_type != "groupavg") %>% 
  group_by(iteration, encoding_type) %>% 
  summarize(q2 = mean(q2), .groups = "drop") %>% 
  left_join(metrics_sc %>% 
              filter(stim_type == "ring_expand") %>% 
              select(fold_num, perf_overall) %>% 
              unnest(perf_overall) %>% 
