...
video_durations = []
video_nframes = []
for root, pwd, files in os.walk(fps10_path):
...
video_metadata.to_csv(os.path.join(metadata_path, 'video_10fps_metadata.csv'))
...
# %%
# make framewise version of video metadata for framewise torch dataset
# only using the 10 fps resampled frames from the short ones!
video_ids = []
frame_ids = []
emotions = []
trains = []
...
for idx, nframes, emotion, train, censored in video_metadata[['nframes', 'emotion', 'train', 'censored']].itertuples():
...
frame_metadata = pd.DataFrame({'emotion': emotions, 'train': trains, 'censored': censoreds}, index=[video_ids, frame_ids])
frame_metadata.index.names = ['video', 'frame']

frame_metadata.to_csv(os.path.join(metadata_path, 'frame_10fps_metadata.csv'))
# %%
